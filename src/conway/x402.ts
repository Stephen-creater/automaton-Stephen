...
function selectRequirement(parsed: PaymentRequiredResponse): PaymentRequirement {
  const exactSupported = parsed.accepts.find(
    (requirement) => requirement.scheme === "exact" && !!CHAINS[requirement.network],
  );
...
export async function checkX402(url: string): Promise<PaymentRequirement | null> {
  try {
    const response = await x402HttpClient.request(url, { method: "HEAD" });
    if (response.status !== 402) {
      return null;
    }
    const parsed = await parsePaymentRequiredResponse(response);
...
export async function x402Fetch(
  ...
  try {
    ...
    const parsed = await parsePaymentRequiredResponse(initialResp);
    if (!parsed) {
      return {
        success: false,
    ...
    try {
      ...
      return {
        success: false,
        error: `Failed to sign payment: ${error?.message || String(error)}`,
        status: initialResp.status,
      };
    }
