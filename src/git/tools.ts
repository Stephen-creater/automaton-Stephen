...
export async function gitStatus(
  ...
  for (const line of lines) {
    ...
    if (statusCode[0] !== " " && statusCode[0] !== "?") staged.push(file);
    if (statusCode[1] === "M" || statusCode[1] === "D") modified.push(file);
...
export async function gitClone(
  ...
  const depthArg = depth ? ` --depth ${Math.max(1, Math.floor(Number(depth)))}` : "";
  const result = await conway.exec(
