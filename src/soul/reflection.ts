...
export async function reflectOnSoul(
  ...
  try {
    ...
    const recentEvidence = gatherRecentEvidence(db);
    const autoUpdated: string[] = [];
    const autoUpdates: Partial<SoulModel> = {};

    const capabilitiesSummary = summarizeCapabilities(recentEvidence.toolsUsed);
    ...
    const relationshipsSummary = summarizeRelationships(recentEvidence.interactions);
    ...
    const financialSummary = summarizeFinancial(recentEvidence.financialActivity);
...
function gatherRecentEvidence(db: BetterSqlite3.Database): RecentEvidence {
  ...
  try {
    ...
    for (const row of toolRows) toolsUsed.push(row.name);
...
function summarizeCapabilities(toolsUsed: string[]): string {
  ...
  return `Tools used: ${[...new Set(toolsUsed)].join(", ")}`;
}
